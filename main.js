...
let positionBottom = parseInt(
  window.getComputedStyle(personagem).bottom.replace('px', '')
);
let positionLeft = parseInt(
  window.getComputedStyle(personagem).left.replace('px', '')
);

const tempo = setInterval(() => {
  let npcLeft = parseInt(window.getComputedStyle(npc).left.replace('px', ''));
  let npcBottom = parseInt(
    window.getComputedStyle(npc).bottom.replace('px', '')
  );

  if (
    positionBottom >= npcBottom - 80 &&
    positionBottom <= npcBottom + 80 &&
    positionLeft <= npcLeft + 80 &&
    positionLeft >= npcLeft - 80
  ) {
    animacaoAtaque(npc);
    bater(npc);
...
document.addEventListener('keyup', e => {
  ...
  let lifePositionLeft = parseInt(
    window.getComputedStyle(life3).left.replace('px', '')
  );
  let lifePositionBottom = parseInt(
    window.getComputedStyle(life3).bottom.replace('px', '')
  );
  ...
  if (e.code == 'Space') {
    ...
    if (
      positionBottom >= lifePositionBottom - 50 &&
      positionBottom <= lifePositionBottom + 50 &&
      positionLeft <= lifePositionLeft + 50 &&
      positionLeft >= lifePositionLeft - 50
    ) {
      pegar();
    }
